...
class starter {
		...
		*/
		while(true){
			...
			if(choice.equalsIgnoreCase("file")){
				...
				System.out.println("What word do you want me to check?");
				String word = sc.nextLine();
				int num = checksArray(arr, word);
				System.out.println("Their were " + num + " " + word+ "s in the text");
			}
			else if(choice.equalsIgnoreCase("common")){
				String he = mostCommon(arr);
			}
	...
	*/
	public static int checksArray(String[] arr, String check){
			int count = 0;
		for(int i = 0; i < arr.length; i ++){
			if(arr[i].equalsIgnoreCase(check)){
				count ++;
			}
		}
	...
	public static String mostCommon(String[] arr){
		...
		for(int i = 0; i < arr.length; i ++){
			for(int y = 0; y < arr.length; y ++){
				if(arr[i].equalsIgnoreCase(arr[y])){
					count[i] ++;
				}
			}
		}
		...
		System.out.println(arr[most] + " was used " + max + " times");
		return arr[most];	// Dummy value, delete when needed.
	}
